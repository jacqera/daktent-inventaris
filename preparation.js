...
export function createPreparation({getState,store,header,nav,search,scopeBar,render,load,toast,openPhoto}){
  let onlyTodo=true,openBox=null,undo=null,busy=false,page='start';
  ...
  function get(){const s=getState();s.data.mode=page==='storage'?'storage':page==='travel'?'travel':'current';return {s,m:model(s.data,s.data.mode),prep:s.data.mode!=='current'};}
  ...
  function results(){
    const {s,m,prep}=get(),eligible=m.rows.filter(r=>!r.excluded),relevant=m.boxes.filter(b=>b.relevant),remaining=eligible.filter(r=>!r.target.box&&!r.done).length;
    const cards=relevant.filter(b=>{const scope=s.filters.current;return (!prep||!onlyTodo||!b.done)&&(!scope||(boxId(scope)?b.box.id===boxId(scope):(prep?b.site:b.box.current_location)===scope))&&(!s.query||b.contents.some(r=>[r.item.name,r.item.description].join(' ').toLowerCase().includes(s.query.toLowerCase()))||b.box.name.toLowerCase().includes(s.query.toLowerCase()));});
    ...
    const undoBar=undo?`<aside class="prep-undo">${e(undo.label)} ${button('undo','','Ongedaan maken')}</aside>`:'';
    if(openBox){
      const b=m.boxes.find(b=>b.box.id===openBox);if(!b)return '<p>Deze box bestaat niet meer.</p>';
      const p=b.box.photos?.find(p=>p.url),rows=b.contents.filter(r=>!prep||!onlyTodo||!r.done);
      return `${undoBar}<section class="prep-open"><div class="box-page-title">${p?button('photo',p.url,`<img src="${e(p.url)}" alt="Foto van Box ${openBox} vergroten">`):''}<div><h2>${e(b.box.name)}</h2><p>Nu: ${e(b.box.current_location||'Onbekend')}${prep?` · Hoort: ${e(b.site||'Niet ingesteld')}`:''}</p></div></div>${prep?`<div class="prep-actions">${button('todo','','Nog doen',`aria-pressed="${onlyTodo}"`)}${button('all','','Alles',`aria-pressed="${!onlyTodo}"`)}</div>`:''}${rows.map(r=>row(r,prep,m)).join('')||'<p>Geen openstaande artikelen. Kies Alles om de volledige inhoud te zien.</p>'}${button('box-details',openBox,'Boxfoto en gegevens wijzigen')}</section>`;
    }
    const start=page==='start';
    return `${prep?`<section class="prep-progress"><strong>${relevant.filter(b=>b.done).length} van ${relevant.length} boxen klaar · ${remaining} losse artikelen nog doen</strong><p>Klaar = op de bedoelde plek én inhoud gecontroleerd.</p>${s.data.mode==='travel'?`<p>${m.trip?`Reis: ${e(m.trip.name)}`:'Kies een actieve reis via Beheer → Reizen om controles en uitzonderingen te bewaren.'}</p>`:''}<div class="prep-actions">${button('todo','','Nog doen',`aria-pressed="${onlyTodo}"`)}${button('all','','Alles',`aria-pressed="${!onlyTodo}"`)}${s.data.mode==='storage'?button('new-round','','Nieuwe stallingsronde'):''}</div></section>`:''}${undoBar}<h2>${start?'Je boxen':'Boxen op deze locatie'}</h2><section class="${start?'box-grid prep-boxes':'compact-boxes'}">${cards.map(b=>{const p=b.box.photos?.find(p=>p.url);return `<article class="prep-box ${start?'':'compact-box'}"><button data-prep="box" data-id="${b.box.id}" class="prep-box-open"><div class="prep-photo">${p?`<img src="${e(p.url)}" alt="${e(b.box.name)}">`:'Geen foto'}</div><div><small>BOX ${b.box.id}</small><h3>${e(b.box.name)}</h3><p>${b.contents.length} artikelen · Nu: ${e(b.box.current_location||'Onbekend')}</p>${prep?`<p>Hoort: ${e(b.site||'Niet ingesteld')} · ${b.atPlace?'Box op de juiste plek':'Box nog verplaatsen'}</p><p>${b.missing} artikelen niet op hun plek · ${b.unchecked} nog controleren</p>`:''}</div></button>${prep?`<div class="prep-actions">${!b.atPlace&&b.site?button('move-box',b.box.id,`Box naar ${e(b.site)} gezet`):''}${button('check-box',b.box.id,'Inhoud controleren')}</div>`:''}</article>`}).join('')||'<p>Geen boxen voor deze selectie.</p>'}</section>${start?`<p class="start-summary">${m.rows.filter(r=>!r.item.box&&matches(r,s)).length} losse artikelen in deze selectie.</p>${button('page','locations','Bekijk locatieoverzicht →')}`:`<section class="prep-overview"><h2>Losse artikelen</h2>${itemRows.filter(r=>!(prep?r.target.box:r.item.box)).map(r=>row(r,prep,m)).join('')||'<p>Geen losse artikelen voor deze selectie.</p>'}</section>${excluded.length?`<details><summary>Deze reis niet mee (${excluded.length})</summary>${excluded.map(r=>row(r,prep,m)).join('')}</details>`:''}`}`;
  }

  function home(){const {s}=get();const title=openBox?`Box ${openBox}`:({start:'Alles mee. Niets vergeten.',locations:'Locatieoverzicht',storage:'Stalling voorbereiden',travel:'Vertrek voorbereiden'}[page]);
    return `${header(title)}${page==='start'&&!openBox?`<section class="page-links" aria-label="Overzichten">${button('page','locations','Locatieoverzicht')}${button('page','storage','Stalling voorbereiden')}${button('page','travel','Vertrek voorbereiden')}</section><p class="version-label">Versie 24 · Tik op een box voor de inhoud</p>`:''}${!openBox?scopeBar().replace('Toon inhoud van',s.data.mode==='current'?'Toon huidige locatie':'Toon bestemming')+search():''}<main id="prep-results">${results()}</main>${nav()}`;
  }
  ...
  function bind(){
    if(getState().view==='trips'){
      ...
      if(host&&m.trip){const rows=m.rows.filter(r=>!r.excluded),progress=host.querySelector('.pack-progress'),list=host.querySelector('.trip-packlist');
        ...
        if(list){list.innerHTML=button('open-preparation','','Open vertrekvoorbereiding');list.querySelector('button').onclick=async event=>{event.stopPropagation();await store.setMode('travel');s.data.mode='travel';page='travel';openBox=null;s.view='home';render();};}
      }
    }
    ...
    const app=document.querySelector('#app'),old=app.onclick;app.onclick=async event=>{const b=event.target.closest('[data-prep]');if(!b)return old(event);event.stopPropagation();if(busy)return;const {s,m}=get(),id=b.dataset.id,action=b.dataset.prep,r=m.rows.find(r=>r.item.id===id);try{
      if(action==='page'||action==='mode'){page=action==='page'?id:({current:'locations',storage:'storage',travel:'travel'}[id]);s.data.mode=page==='storage'?'storage':page==='travel'?'travel':'current';await store.setMode(s.data.mode);openBox=null;render();return;}
      if(action==='photo'){openPhoto(id);return;}
      if(action==='todo'||action==='all'){onlyTodo=action==='todo';render();return;}
      if(action==='box'||action==='close-box'){openBox=action==='box'?Number(id):null;render();document.querySelector('.prep-open')?.scrollIntoView({block:'start',behavior:'smooth'});return;}
      ...
      if(action==='check-box'){const box=m.boxes.find(x=>x.box.id===Number(id));openBox=box.box.id;busy=false;render();document.querySelector('.prep-open')?.scrollIntoView({block:'start'});toast('Controleer de artikelen één voor één; de box wordt vanzelf klaar.');return;}
  ...
  return {home,bind,normalize(){if(getState().view==='home')get();},route(){return {page,box:openBox,todo:onlyTodo};},restore(r){page=r?.page||'start';openBox=r?.box||null;onlyTodo=r?.todo!==false;}};
}
